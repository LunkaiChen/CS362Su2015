...
void smithyCard(struct gameState *state, handpos){
	currentPlayer = whoseTurn(state)
	for (i = 0; i < 6 i++)
	{
...
void adventureCard(struct gameState *state, currentPlayer){
	...
	void council_room_card(struct gameState *state, handpos){
		currentPlayer = whoseTurn(state);
		for (i = 0; i < 4; i++)
		{
			drawCard(currentPlayer, state);
	...
	void villageCard(struct gameState *state, handpos){
		currentPlayer = whoseTurn(state);
		//+1 Card
		drawCard(currentPlayer, state);
	...
	void great_hallCard(struct gameState *state, handpos){
		currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	int thisDeckCount = state->deckCount[currentPlayer];
	int 
	switch (card)
	{
	case adventurer:
		adventureCard(state, currentPlayer);
			return 0;
	case council_room:
		counsil_room(state, handpos);
