...
void smithyCard(struct gameState *state, int currentPlayer, int handPos){
	...
	int i;
        ...
        //The Smithy Card is supposed to draw 3 cards, but will only draw 2.  
	for (i = 0; i < 2; i++){  
...
void adventurerCard(struct gameState *state, int currentPlayer){
    ...
    while(drawntreasure < 2){  //loop until 2 treasures are found
        ...
        if (cardDrawn == copper ||  cardDrawn  == silver || cardDrawn == gold){  
            ...
            drawntreasure--;  //drawn treasure is decremented when it should be incremented
...
void villageCard(struct gameState *state, int currentPlayer, int handPos){
      ...
      //+1 Card
      drawCard(currentPlayer, state);
      drawCard(currentPlayer, state);     //will draw twice instead of once
      
      //+2 Actions
      state->numActions = handPos + 2;  
...
void stewardCard(int choice1, int choice2, int choice3, struct gameState *state, int currentPlayer, int handPos){
      ...
      if (choice1 == 1)
        ...
        {
          //+2 coins
          state->coins = state->coins + 5;  //bug adds 5 coins instead of 2
        }
...
void councilRoomCard(struct gameState *state, int currentPlayer, int handPos){
	...
	{
	  if ( i = currentPlayer )    //bug draws card for wrong player
	    {
	      drawCard(i, state);
	    }
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
    	stewardCard(choice1, choice2, choice3, state, handPos, bonus);
    	return 0;
