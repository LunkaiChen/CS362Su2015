...
int council_roomEffect(struct gameState *state, int currentPlayer,int handPos)
{
      ...
      for (i = 0; i < 4; i++)
	{
      ...
      //+1 Buy
      state->numBuys+=2;                         ////ERROR: Numbuys is to be increased by only 1 but here is increased by 2
...
int feastEffect(struct gameState *state, int currentPlayer, int temphand[],int choice1)
{
	...
	state->hand[currentPlayer][i] = -1;//Set to nothing
      ...
      //Condition to loop on                        /////ERROR, x should have been intialized to 1
...
int stewardEffect(struct gameState *state, int currentPlayer, int choice1, int choice2, int choice3, int handPos)
{
    ...
    return 0;
      //discard card from hand                                  //////ERROR: have to discard card from hard, that line is missing
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case steward:
        stewardEffect(state, currentPlayer, choice1, choice2, choice3, handPos);
      return 0;
