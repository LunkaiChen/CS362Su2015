...
void adventurerRefactored (int passed, int currentPlayer, struct gameState *state)
{
  int drawntreasure = passed;
  int z = 0;
...
void smithyRefactored(int currentPlayer, struct gameState *state)
{
  int i;
  for(i = 0; i < 3; i++)
  {
...
void councilRoomRefactored(int currentPlayer, int handPos, struct gameState *state)
{
      //+4 Cards
  int i, j;
      for (i = 0; i < 4; i++)
  {
    drawCard(currentPlayer, state);
      ...
      for (j = 0; j < state->numPlayers-1; j++)
  {
    if ( j != currentPlayer )
      {
        drawCard(j, state);
...
void greatHallRefactored(int currentPlayer, int handPos, struct gameState *state)
{
     //+1 Card
      drawCard(currentPlayer, state);
...
void stewardRefactored(int currentPlayer, int choice1, int choice2, int choice3, int handPos, struct gameState *state)
{
      if (choice1 == 1)
  {
    //+2 cards
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
 ...
 // int z = 0;// this is the counter for the temp hand
  ...
  switch( card ) 
    {
    case adventurer:
      adventurerRefactored(drawntreasure, currentPlayer, state);
			
    case council_room:
      councilRoomRefactored(currentPlayer, handPos, state);
    ...
    case smithy:
      smithyRefactored(currentPlayer, state);
    ...
    case great_hall:
      greatHallRefactored(currentPlayer, handPos, state);
    ...
    case steward:
        stewardRefactored(currentPlayer, choice1, choice2, choice3, handPos, state);
