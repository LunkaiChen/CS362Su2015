...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
		return playMine(state, choice1, choice2);  
...
void playAdventurer (struct gameState *state) {
	...
	while(pos-1>=0){
		state->discard[currentPlayer][state->discardCount[currentPlayer]++]=temphand[pos-1]; // discard all cards in play that have been drawn
		pos=pos - 1;
    }
}

void playCouncil(struct gameState *state) {
